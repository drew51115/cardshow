...
26a. **`#fabBulkScan` — secondary mobile FAB, stacked above `#fabAddCard`** — 44px circle (vs. the primary FAB's 56px), centered on the same right edge, `bottom: 6.5rem` so it sits just above the main FAB with a visible gap. Same seller-only show/hide lifecycle as `#fabAddCard` (all three toggle points: `loginAsSeller()`, `loginAsAdmin()`, `signOut()`). `onclick` just triggers the same hidden `#bulkScanInput` the sidebar button uses — no separate scan logic. Added because the sidebar (where Bulk Scan otherwise lives) is collapsed by default on mobile (`initSidebarState()`), making it easy to miss; sellers photographing showcases at a show benefit from one-tap access without hunting through "☰ Tools & Stats". `handleBulkScanFile()` disables both `#bulkScanBtn` and `#fabBulkScan` for the duration of a scan (the FAB has no text label, so it just dims via `#fabBulkScan:disabled`) regardless of which one triggered it, so a second tap mid-scan can't fire the file picker again.
