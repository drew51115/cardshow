...
- `_orgFetchTableAssignments(showId)` — `show_sellers.select('seller_id, table_number')`; returns `{ seller_id: table_number }`, degrades to `{}` on error
- `_orgRender(showId, cards, events, tableMap)` — computes all metrics, calls sub-renderers
