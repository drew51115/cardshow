...
### PSA/CGC verify button (on-demand, seller-initiated)
...
- **Deliberately seller-initiated per card, not automatic per scan.** PSA's free tier is ~10 req/hr (see Known Constraints below) — a busy showcase photo can easily have more graded slabs than that in one shot, so auto-verifying every card would exhaust the quota in a single scan. The seller decides which cards are worth spending a lookup on.
- Only helps graded slabs (a cert number requires a slab label to exist). Does not address raw-card misidentification.
