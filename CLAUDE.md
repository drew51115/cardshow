...
3. **The plain (non-bracketed) `full_name=`, `last_name=`, and `name=` params are real and do filter — but not as a live-typing prefix search.** `GET /v1/players?full_name=` only prefix-matches when the query is a *single* word (`full_name=paul` → 583 matches, genuinely a prefix search); the moment a second word is present, that word must be a complete, exact token or the whole query returns zero (`full_name=paul sk` → 0, `full_name=paul skenes` → 1). `GET /v1/sets?name=` is stricter still — it requires the *entire* value to exactly match a set's name; even a single complete word alone (`name=Bowman`) returns zero against a real set like "2025 Bowman Chrome". Neither behavior is documented in the OpenAPI spec, and both were only found by reproducing a real failure end-to-end against the live deploy (see the two features below for how each routes around it) — reasoning about encoding/URL correctness in the abstract was not enough to catch this; it took an actual before/after curl comparison against production to confirm what actually changed the result.
...
- **Step 1 — Player typeahead** (`#ac_player`, `tcPlayerTypeahead()`): 3+ chars, 400ms debounce, `_tcSearchPlayers(q)` — tries `GET /v1/players?full_name={q}` first; if that's empty (the seller is still mid-typing a multi-word name, e.g. `"paul sk"`, which the API's word-not-prefix matching rejects — see the API constraint above), retries with just the first word. This prevents the "no matches" dead-end while typing, but doesn't guarantee the wanted player appears in the capped 8-result dropdown for a large/common first name — the exact multi-word match only becomes reliable once the full name is typed and paused on. `_tcSearchPlayers()` is shared with the bulk-scan validator below. Selecting a result fills Player + `acPlayerUUID`, clears all downstream UUIDs, and focuses the Set field.
...
2. Best-effort enrichment once a player matches (adapted from the spec's broken `filter[player_id]` approach): `_tcGetAllSets()` (same session-cached full catalog as the picker's Step 2 — `GET /v1/sets?name=` is exact-match-only and unusable here, a vision-extracted set name is very unlikely to match it character-for-character) filtered client-side by substring, preferring a candidate whose name also contains the card's year when one matches, falling back to a year-agnostic substring match otherwise → `GET /v2/sets/{id}/checklist` (fills Card # if empty, matched by player-name substring) and `GET /v1/sets?parent_id={id}` in parallel (fuzzy-matches and normalizes the Parallel field). Any failure here is silent/non-fatal and never downgrades an already-VERIFIED badge.
