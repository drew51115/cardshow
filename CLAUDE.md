...
- **show.html fetchInventoryFromDB() two-step query fix (session 2026-08-28)** — real-world symptom: a show with 5 published cards showed only 1 to buyers. Root cause: `fetchInventoryFromDB()` queried `show_inventory` with a nested embedded-join select (`.select('inventory(id, ..., sellers(handle))')`) — the exact pattern already documented elsewhere in this file (`loadShowSellersAndTables()`, `_orgFetchInventory()`) as returning `null` unpredictably for some rows under a stale PostgREST schema cache. That failure is row-by-row, not all-or-nothing, so it presents as "some published cards missing" rather than a clean empty result — matching the reported symptom exactly. Rewrote to the same two explicit queries `_orgFetchInventory()` already uses (`show_inventory.select('card_id')` → `inventory.in('id', cardIds)`), plus one batched `sellers.select('id, handle').in('id', sellerIds)` lookup to replace the embedded join's inline `sellers(handle)`. No schema or field changes — same `dbCards` shape as before.
