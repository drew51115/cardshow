...
- `scorePCResult(product, card)` scores PriceCharting results before selection — all 100 returned products are scored and sorted; the highest score wins. Scoring: bracket variants like `[Image Variation]` / `[X-Fractor]` penalised -25 unless bracket content matches seller's parallel (+5); base cards (no brackets) get +15; card number match in product name +20; last name match +5; subset keywords (image variation, rookie cup, award winner, variation, etc.) -15.
